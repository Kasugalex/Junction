...
namespace Main
{
    class JunctionPath
    {
        private static string Title = "rem mklink";
        private static string Templete = "mklink /j \"{0}\" \"{1}\"";
        ...
        public static void Junction()
        {
            ...
            string saveTemplete = "链接终点:{0}    链接起点:{1}    链接时间:{2}";
            using (FileStream stream = new FileStream(saveTxt, FileMode.Append, FileAccess.Write))
            {
                ...
                foreach (var item in paths)
                {
                    string context = string.Format(saveTemplete, item.Key, item.Value,DateTime.Now.ToString("yyyy-mm-dd HH:mm:ss"));
                    writer.WriteLine(context);
                }
                writer.Flush();
            }
